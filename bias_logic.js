...
function renderWinningTime() {
    ...
    const wtClass   = document.getElementById('wt-class').value || 'クラス(すべて)';
    ...
    const rows = [];
    function collectRowsForSurface(surf) {
        const surfData = venueData[surf] || {};
        for (const dist in surfData) {
            const classData = surfData[dist] || {};
            const condMap = classData[wtClass] || {};
    ...
    if (surface) {
        collectRowsForSurface(surface);
    } else {
        for (const surf in venueData) collectRowsForSurface(surf);
    }
